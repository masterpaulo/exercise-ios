...
@property (nonatomic) UInt32 availableEncoder;
...
@property (nonatomic) NSMutableData *dataToSave;

@property (assign, getter=shouldBeEncoding) BOOL encode;
@property(nonatomic) uint64_t videoTS;
...
-(void)encodeSample:(CMSampleBufferRef)sampleBuffer {
    ...
    if (self.shouldBeEncoding) {
        ...
        uint64_t ptsMicro =
        (uint64_t) (((double)pts.value / pts.timescale) * 1000000);

        SInt64 ptsMicroAdjusted = CFSwapInt64HostToBig(ptsMicro);
        //        NSLog(@"Audio TS %llu",ptsMicro);
        [self encodeSampleBuffer:sampleBuffer completionBlock:^(NSData *encodedData, NSError *error) {
            
            NSMutableData *audioPacket = [[NSMutableData alloc] init];
            [audioPacket appendBytes:&ptsMicroAdjusted length:8];
            [audioPacket appendData:encodedData];
            
            [self.receiver sendAudioData:audioPacket];
        }];
    }
}
...
- (void)initializeVars {
    ...
    if (IsAACEncoderAvailable(_availableEncoder)) {
        NSLog(@"AAC Encoder available: %u", (unsigned int)_availableEncoder);
    }else{
        NSLog(@"AAC Encoder NOT available!: %u", (unsigned int)_availableEncoder);
    }
}
...
- (void) setupEncoderFromSampleBuffer:(CMSampleBufferRef)sampleBuffer {
    ...
    outAudioStreamBasicDescription.mSampleRate = 8000;//inAudioStreamBasicDescription.mSampleRate;
    
    NSLog(@"mSampleRate %f", inAudioStreamBasicDescription.mSampleRate);
    // The number of frames per second of the data in the stream, when the stream is played at normal speed. For compressed formats, this field indicates the number of frames per second of equivalent decompressed data. The mSampleRate field must be nonzero, except when this structure is used in a listing of supported formats (see “kAudioStreamAnyRate”).
    outAudioStreamBasicDescription.mFormatID = kAudioFormatMPEG4AAC; // kAudioFormatMPEG4AAC_HE does not work. Can't find `AudioClassDescription`. `mFormatFlags` is set to 0. //kLinearPCMFormatFlagIsPacked
    //    outAudioStreamBasicDescription.mFormatFlags = kAudioFormatFlagIsSignedInteger | kAudioFormatFlagIsPacked;// Format-specific flags to specify details of the format. Set to 0 to indicate no format flags. See “Audio Data Format Identifiers” for the flags that apply to each format.
    ...
    UInt32 encoderType;
    IsAACEncoderAvailable(encoderType);
    AudioClassDescription *description = [self
                                          getAudioClassDescriptionWithType:kAudioFormatMPEG4AAC
                                          fromManufacturer:kAppleSoftwareAudioCodecManufacturer];//kAppleSoftwareAudioCodecManufacturer
    ...
    if (status != noErr) {
        NSError *error = [NSError errorWithDomain:NSOSStatusErrorDomain
                                             code:status
                                         userInfo:nil];
        NSLog(@"Error AudioConverterNewSpecific: ln 146 %@", [error description]); //kAudioFormatUnsupportedDataFormatError
    }else{
        NSLog(@"NO Error AudioConverterNewSpecific");
    }
}
...
static OSStatus inInputDataProc(AudioConverterRef inAudioConverter, UInt32 *ioNumberDataPackets, AudioBufferList *ioData, AudioStreamPacketDescription **outDataPacketDescription, void *inUserData)
{
    ...
    if (copiedSamples < requestedPackets) {
        //        NSLog(@"PCM buffer isn't full enough! %d", (unsigned int)copiedSamples);
...
- (void) encodeSampleBuffer:(CMSampleBufferRef)sampleBuffer completionBlock:(void (^)(NSData * encodedData, NSError* error))completionBlock {
    ...
    if (status == 0) {
        ...
        //            NSLog(@"Size: %lu", (unsigned long)[rawAAC length]);
        data = fullData;
...
static Boolean IsAACEncoderAvailable(UInt32 encoderType)
{
    ...
    for (UInt32 i=0; i < numEncoders; ++i) {
        if (encoderDescriptions[i].mSubType == kAudioFormatMPEG4AAC && encoderDescriptions[i].mManufacturer == kAppleHardwareAudioCodecManufacturer) {
            ...
            isAvailable = true;
            encoderType = kAppleHardwareAudioCodecManufacturer;
        }
        if (encoderDescriptions[i].mSubType == kAudioFormatMPEG4AAC && encoderDescriptions[i].mManufacturer == kAppleSoftwareAudioCodecManufacturer) {
            ...
            isAvailable = true;
            encoderType = kAppleSoftwareAudioCodecManufacturer;
        }
    }
